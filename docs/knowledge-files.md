...
В шестом срезе добавлен браузерный attachment client «База знаний → Файлы»:
[`knowledge-file-client.md`](knowledge-file-client.md). Он читает выбранный человеком файл,
вычисляет hash/base64, отправляет запрос через личную session и показывает статус/историю.
Отдельный CLI сотруднику не нужен; произвольного доступа к локальному диску у MCP по-прежнему нет.
Citation содержит nullable `source_file_id`;
