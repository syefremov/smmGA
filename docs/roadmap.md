...
- украденный/истёкший/отозванный token отвергается;
...
- пользователь без Publisher/Owner permission не одобряет и не планирует публикацию;
...
На момент создания roadmap завершена только документальная часть подготовки: архитектура, product defaults, RAG, web, AI workflows, Git-регламент и `AGENTS.md`. Исполняемого каркаса, lockfiles, bootstrap-скриптов и развёрнутого сервера ещё нет, поэтому ни одна из десяти фаз пока не считается закрытой.

Следующая практическая итерация — начать фазу 1 с read-only аудита основной Windows-машины, затем добавить version-файлы, `bootstrap-dev.ps1`, `doctor.ps1` и `docs/local-development.md`. Установка или изменение серверных служб начинается только в фазе 3 и только после восстановления надёжного SSH/recovery-доступа.
