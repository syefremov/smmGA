...
Пятый срез: [`ingestion-jobs.md`](ingestion-jobs.md) — отмена и приватный список ingestion jobs,
неизменяемая история переходов, bounded reconciliation после сбоев/отзыва доступа/expiry,
явный retry/rescan/reindex. Этот пункт остатка фазы 7 закрыт; внешние gates сохраняются.

Шестой срез: [`knowledge-file-client.md`](knowledge-file-client.md) — attachment client
«База знаний → Файлы» без установки дополнительных программ сотруднику. Загрузка через
...
Десятый срез: [`editor-triage.md`](editor-triage.md) — human triage замечаний по exact
artifact/revision/finding hashes, версиям и явному подтверждению. История не перезаписывается,
повторы/конфликты между машинами проверяются. Нет автоматических правок или одобрения поста.
...
Четырнадцатый срез: [`planner-adoption.md`](planner-adoption.md) — exact human preview/consent
сохраняет новую draft-версию плана, общие основания/пробелы и private provenance receipt.
